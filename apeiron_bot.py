...
async def load_cogs():
        ...
        for filename in files:
            ...
            if filename.endswith(".py") and not filename.startswith("_"):
                ...
                try:
                    ...
                    logger.error(
                        f"⚠️ '{module_name}' のロードに失敗しました: {e}", exc_info=True
                    )
    ...
    try:
        if "cogs.events.event_manager.cog" not in bot.extensions:
            ...
            logger.info(
                "フォールバックで 'cogs.events.event_manager.cog' をロードしました。"
            )
    except Exception as e:
        logger.error(
            "フォールバックで event_manager のロードに失敗しました", exc_info=True
        )
...
async def help2(ctx):
    ...
    embed.add_field(
        name="基本操作",
        value="**`!通話`**: Botをボイスチャンネルに呼び出します。\n**`!再生 [曲名 or URL]`**: 曲を検索して再生リストに追加します。\n**`!退出`**: Botをボイスチャンネルから退出させます。",
        inline=False,
    )
    embed.add_field(
        name="再生コントロール",
        value="**`!一時停止`**, **`!再開`**, **`!スキップ`**, **`!停止`**",
        inline=False,
    )
    embed.add_field(
        name="再生リスト管理",
        value="**`!一覧`**: 現在の再生リストを表示します。",
        inline=False,
    )
    await ctx.send(embed=embed)
...
async def help4(ctx):
    ...
    embed.add_field(
        name="【推奨】パネルからの操作",
        value="指定されたチャンネルにあるパネルのボタンから、直感的にほとんどの機能を利用できます。\n・**手動登録**: 1戦ずつ戦績を記録します。\n・**戦績表示**: 戦績サマリーを表示します。\n・**直近履歴**: 最新の対戦履歴を表示します。\n・**通知チャンネル設定**: 表示先チャンネルを変更します。\n・**全データ削除**: データを削除します。",
        inline=False,
    )
    embed.add_field(
        name="スラッシュコマンド",
        value="・**/replay [image]**: リプレイ画像から一括登録します。\n・**/record**, **/stats**, **/history** も利用可能です。",
        inline=False,
    )
    embed.add_field(
        name="テキストコマンド",
        value="**`!panel`**: 操作パネルを再設置します。",
        inline=False,
    )
    await ctx.send(embed=embed)
...
async def help5(ctx):
    ...
    embed.add_field(
        name="【基本】パネルからの操作",
        value="管理者が `!総当たりパネル` で設置したパネルから操作します。\n・**チーム作成**: チームを作りリーダーになります。\n・**チームに参加/脱退**: チームへの出入りを行います。",
        inline=False,
    )
    embed.add_field(
        name="コマンド管理",
        value="**`!チーム登録`**, **`!チームメンバー追加`**, **`!チーム取消`**",
        inline=False,
    )
    embed.add_field(
        name="大会進行",
        value="**`!総当たり開始`**, **`!次節`**, **`!順位`**, **`!総当たり中止`**",
        inline=False,
    )
    await ctx.send(embed=embed)
...
async def on_message(message: discord.Message):
    ...
    try:
        if isinstance(message.content, str) and message.content.startswith("!"):
            logging.getLogger("cogs").info(
                f"受信メッセージ: author={message.author} channel={getattr(message.channel, 'id', None)} content={message.content!r}"
            )
    except Exception:
        pass
    # コマンド処理を続行
