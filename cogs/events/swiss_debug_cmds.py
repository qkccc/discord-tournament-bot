...
class SwissDebugCog(commands.Cog, name="SwissDebug"):
    ...
    async def swiss_status(self, ctx: commands.Context):
        ...
        tournament = db.fetchone(
            "SELECT * FROM swiss_tournaments WHERE guild_id = ?",
            (guild_id,)
        )
    ...
    async def swiss_pairings(self, ctx: commands.Context):
        ...
        for i, pair in enumerate(pairings, 1):
            ...
            ) if pair['player2_id'] else None

            p1_name = p1['display_name'] if p1 else f"ID: {pair['player1_id']}"
            p2_name = p2['display_name'] if p2 else ("Bye" if not pair['player2_id'] else f"ID: {pair['player2_id']}")
    ...
    @commands.has_permissions(administrator=True)
    async def swiss_results(self, ctx: commands.Context, round_num: int = None):
        """スイスドロー結果を表示"""
        guild_id = ctx.guild.id
        ...
        if round_num is None:
            ...
            results = db.fetchall(
                "SELECT round_num, winner_id, loser_id FROM swiss_results WHERE guild_id = ? AND round_num = ?",
                (guild_id, round_num)
            )
        ...
        for result in results:
            ...
            winner_name = winner['display_name'] if winner else f"ID: {result['winner_id']}"
    ...
    async def swiss_export(self, ctx: commands.Context):
        ...
        players = db.fetchall(
            "SELECT * FROM event_players WHERE guild_id = ?",
            (guild_id,)
        )

        pairings = db.fetchall(
            "SELECT * FROM swiss_pairings WHERE guild_id = ?",
            (guild_id,)
        )

        results = db.fetchall(
            "SELECT * FROM swiss_results WHERE guild_id = ?",
            (guild_id,)
        )
    ...
    async def swiss_verify(self, ctx: commands.Context):
        ...
        for pair in pairings:
            if pair['player2_id'] and pair['player1_id'] == pair['player2_id']:
                pairing_issues.append(f"⚠️ 無効なペア: {pair['player1_id']} vs {pair['player1_id']}")

        if pairing_issues:
            checks.append("❌ ペアリング エラー:\n" + "\n".join(pairing_issues[:5]))
        else:
