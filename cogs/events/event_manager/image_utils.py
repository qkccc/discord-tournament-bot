...
def create_bracket_image_from_db(guild_id: int, db: DatabaseManager) -> discord.File:
    ...
    tournament_info = db.fetchone(
        "SELECT * FROM se_tournaments WHERE guild_id = ?", (guild_id,)
    )
    if not tournament_info:
        return None

    matches_data = db.fetchall(
        "SELECT * FROM se_matches WHERE guild_id = ? ORDER BY round_num, match_in_round",
        (guild_id,),
    )
    players_data = db.fetchall(
    ...
    base_slot_centers_y = [
        padding + (i + 0.5) * slot_y_increment for i in range(tourney_size)
    ]

    for match in matches_data:
        r_idx = match["round_num"] - 1
        winner_box_center_x = (
            padding + box_w + h_gap + r_idx * (box_w + h_gap) + (box_w / 2)
        )
        ...
        if r_idx == 0:
            ...
            match_centers[match["match_id"]] = (
                winner_box_center_x,
                winner_box_center_y,
            )

            p1_name = player_map.get(match["player1_id"], " ")
            p2_name = player_map.get(match["player2_id"], " ")
            is_p1_winner = match["winner_id"] == match["player1_id"]
            is_p2_winner = match["winner_id"] == match["player2_id"]
            if match["is_bye"]:
            ...
            draw.text(
                (player_box_center_x - box_w / 2 + 10, p1_center_y - box_h / 2 + 10),
                p1_name,
                font=font,
                fill=winner_font_color if is_p1_winner else font_color,
            )
            draw.rectangle(
                (
                    player_box_center_x - box_w / 2,
            ...
            draw.text(
                (player_box_center_x - box_w / 2 + 10, p2_center_y - box_h / 2 + 10),
                p2_name,
                font=font,
                fill=winner_font_color if is_p2_winner else font_color,
            )
        else:
        ...
        draw.text(
            (
                winner_box_center_x - box_w / 2 + 10,
                winner_box_center_y - box_h / 2 + 10,
            ),
            winner_name,
            font=font,
            fill=winner_font_color,
        )
    ...
    for r_idx in range(num_rounds):
        ...
        for match in round_matches:
            ...
            if r_idx == 0:  # 1回戦 -> 1回戦勝者ボックスへの線
                ...
                draw.line(
                    [(source_x_right, p1_center_y), (mid_x, p1_center_y)],
                    fill=line_color,
                    width=line_w,
                )
                draw.line(
                    [(source_x_right, p2_center_y), (mid_x, p2_center_y)],
                    fill=line_color,
                    width=line_w,
                )
                draw.line(
                    [(mid_x, p1_center_y), (mid_x, p2_center_y)],
                    fill=line_color,
                    width=line_w,
                )
                draw.line(
                    [
                        (mid_x, target_center[1]),
                        (target_center[0] - box_w / 2, target_center[1]),
                    ],
                    fill=line_color,
                    width=line_w,
                )
                ...
                if not s1_center or not s2_center:
                    continue
                mid_x = target_center[0] - box_w / 2 - h_gap / 2
                draw.line(
                    [(s1_center[0] + box_w / 2, s1_center[1]), (mid_x, s1_center[1])],
                    fill=line_color,
                    width=line_w,
                )
                draw.line(
                    [(s2_center[0] + box_w / 2, s2_center[1]), (mid_x, s2_center[1])],
                    fill=line_color,
                    width=line_w,
                )
                draw.line(
                    [(mid_x, s1_center[1]), (mid_x, s2_center[1])],
                    fill=line_color,
                    width=line_w,
                )
                draw.line(
