...
class EventManagerCog(commands.Cog, name="EventManager"):
    def __init__(self, bot):
        ...
        try:
            ...
            self.DEFAULT_MATCH_CHANNEL_ID = int(
                os.getenv("DEFAULT_MATCH_CHANNEL_ID", 0)
            )
    ...
    async def _send_ready_se_match_cards(
        self, guild_id: int, match_ch: discord.TextChannel
    ):
        """対戦可能なSEマッチの勝利報告カードを送信する"""
        for match in self.db.fetchall(
            "SELECT * FROM se_matches WHERE guild_id = ? ORDER BY round_num, match_in_round",
            (guild_id,),
        ):
            ...
            if not p1 or not p2:
                log.warning(
                    f"Guild {guild_id}: 対戦カード送信時に参加者を解決できませんでした (match_id={match['match_id']})"
                )
                continue

            if await self._se_match_card_exists(
                match_ch, match["round_num"], p1.display_name, p2.display_name
            ):
                continue
    ...
    async def _load_active_recruitments(self):
        ...
        for session_row in sessions_data:
            ...
            if not guild:
                log.warning(
                    f"Guild ID {guild_id} が見つからないため、募集セッションを復元できませんでした。"
                )
                continue

            # 修正: テーブル名 event_participants
            participants_data = self.db.fetchall(
                "SELECT * FROM event_participants WHERE guild_id = ?", (guild_id,)
            )
            ...
            for p_row in participants_data:
                if p_row["is_dummy"]:
                    ...
                    if member:
                        ...
                        log.warning(
                            f"参加者(ID: {p_row['user_id']})がサーバーに見つかりませんでした。"
                        )
    ...
    async def _load_swiss_tournaments_from_db(self):
        ...
        tournaments_data = self.db.fetchall(
            "SELECT * FROM swiss_tournaments WHERE is_active = ?", (True,)
        )

        for row in tournaments_data:
            guild_id, is_active, round_num, max_rounds = (
                row["guild_id"],
                row["is_active"],
                row["round_num"],
                row["max_rounds"],
            ...
            if not guild:
                log.warning(
                    f"Guild ID {guild_id} のサーバーが見つからないため、スイスドロー大会を復元できませんでした。"
                )
                continue
            ...
            for p_row in players_data:
                ...
                if not member_obj:
                    log.warning(
                        f"Player ID {user_id} ({display_name}) がサーバーに見つかりません。ダミーとして扱います。"
                    )
            ...
            for pair_row in pairings_data:
                ...
                p2 = (
                    tournament.get_player(pair_row["player2_id"])
                    if pair_row["player2_id"]
                    else None
                )
                if p1:
            ...
            reports_data = self.db.fetchall(
                "SELECT winner_id, loser_id FROM swiss_results WHERE guild_id = ? AND round_num = ?",
                (guild_id, round_num),
            )
            for report_row in reports_data:
                tournament.reported_matches_this_round.append(
                    (report_row[0], report_row[1], report_row[0])
                )
    ...
    async def _update_recruitment_message(self, guild_id: int):
        ...
        try:
            ...
            p_list = (
                "\n".join(
                    f"・ {p.display_name}"
                    for p in sorted(list(participants), key=lambda x: x.display_name)
                )
                if participants
                else "(まだ誰もいません)"
            )
    ...
    async def _close_recruitment(
        self,
        interaction: discord.Interaction,
        new_content: str = "募集を締め切りました。",
    ) -> Set[Union[discord.Member, DummyPlayer]]:
        guild_id = interaction.guild.id
        session = self.recruit_sessions.pop(guild_id, None)
        if session:
            # 修正: テーブル名 event_recruitments, event_participants
            self.db.execute(
                "DELETE FROM event_recruitments WHERE guild_id = ?", (guild_id,)
            )
            self.db.execute(
                "DELETE FROM event_participants WHERE guild_id = ?", (guild_id,)
            )
            try:
                channel = self.bot.get_channel(session["channel_id"])
                if not channel:
                ...
                message = await channel.fetch_message(session["message_id"])
                await message.edit(
                    content=f"**--- {new_content} ---**", view=None, embed=None
                )
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        if (
            not session
            or payload.message_id != session["message_id"]
    ...
    @commands.command(
        name="募集",
        aliases=["スイスドロー"],
        help="各種イベントの参加者募集を開始します",
    )
    async def recruit(self, ctx: commands.Context):
        guild_id = ctx.guild.id
        if guild_id in self.recruit_sessions:
            return await ctx.send("現在、既に参加者募集中です。")
        if self.db.fetchone(
            "SELECT 1 FROM se_tournaments WHERE guild_id = ? AND is_active = 1",
            (guild_id,),
        ):
            return await ctx.send(
                "現在進行中のトーナメントがあります。先に`!中止`コマンドで終了してください。"
            )
        if guild_id in self.swiss_tournaments:
            return await ctx.send(
                "現在進行中のスイスドロー大会があります。先に`!中止`コマンドで終了してください。"
            )

        embed = discord.Embed(
            title="🏆 イベント参加者募集 🏆",
            description="このメッセージに👍でリアクションして参加してください！\n\n**現在の参加者 (0人):**\n(まだ誰もいません)",
            color=0x7289DA,
        )
    ...
    async def _get_channels(
        ...
        settings = self.db.fetchone(
            "SELECT main_channel_id, match_channel_id FROM event_settings WHERE guild_id = ?",
            (guild_id,),
        )
        main_ch_id = (
            settings["main_channel_id"]
            if settings and settings["main_channel_id"]
            else self.DEFAULT_MAIN_CHANNEL_ID
        )
        match_ch_id = (
            settings["match_channel_id"]
            if settings and settings["match_channel_id"]
            else self.DEFAULT_MATCH_CHANNEL_ID
        )
    ...
    async def set_main_channel(self, ctx: commands.Context):
        # 修正: テーブル名 event_settings
        self.db.execute(
            "INSERT INTO event_settings (guild_id, main_channel_id) VALUES (?, ?) ON CONFLICT(guild_id) DO UPDATE SET main_channel_id = excluded.main_channel_id",
            (ctx.guild.id, ctx.channel.id),
        )
        await ctx.send(
            f"✅ アナウンスチャンネルを `#{ctx.channel.name}` に設定しました。"
        )

    @commands.command(
        name="sd設定対戦", help="対戦カード送信用チャンネルを設定します。"
    )
    @commands.has_permissions(manage_guild=True)
    async def set_match_channel(self, ctx: commands.Context):
        # 修正: テーブル名 event_settings
        self.db.execute(
            "INSERT INTO event_settings (guild_id, match_channel_id) VALUES (?, ?) ON CONFLICT(guild_id) DO UPDATE SET match_channel_id = excluded.match_channel_id",
            (ctx.guild.id, ctx.channel.id),
        )
        await ctx.send(
            f"✅ 対戦カード送信用チャンネルを `#{ctx.channel.name}` に設定しました。"
        )

    @commands.command(
        name="トーナメント再送",
        help="進行中のトーナメントの対戦カードを全件再送します。",
    )
    @commands.has_permissions(manage_guild=True)
    async def resend_missing_tournament_cards(self, ctx: commands.Context):
        ...
        async for match in self._iter_ready_se_matches(guild_id):
            ...
            view = ResultReportView(
                guild_id, match["match_id"], match["p1_obj"], match["p2_obj"]
            )
            await match_ch.send(embed=embed, view=view)
            sent_count += 1
    ...
    @commands.command(
        name="追加", help="募集中のリストにダミーの参加者を追加します。例: !追加 太郎"
    )
    @commands.has_permissions(manage_guild=True)
    async def add_dummy(self, ctx: commands.Context, *, name: str = None):
        session = self.recruit_sessions.get(ctx.guild.id)
        if not session:
            return await ctx.send("参加者募集中のイベントがありません。")

        if name:
            if any(
                isinstance(p, DummyPlayer) and p.display_name == name
                for p in session["participants"]
            ):
                return await ctx.send(
                    f"エラー: `{name}` という名前のダミー参加者は既に追加されています。",
                    ephemeral=True,
                )
            dummy_name = name
        else:
    ...
    async def remove_dummy(self, ctx: commands.Context, num_to_remove: int = 0):
        ...
        if not dummies:
            return await ctx.send("削除できるダミー参加者がいません。")
    ...
    async def _execute_leader_team_split(
        self, interaction: discord.Interaction, leaders: List[discord.Member]
    ):
        participants = await self._close_recruitment(
            interaction, "チーム分けを実行しました！"
        )
        if not participants:
            return await interaction.followup.send(
                "参加者がいないため、開始できませんでした。", ephemeral=True
            )
        valid_leaders = [
            leader for leader in leaders if leader and leader in participants
        ]
        if len(valid_leaders) != len(leaders):
            return await interaction.followup.send(
                "指定されたリーダーの一部が参加者にいません。", ephemeral=True
            )
        members = list(participants - set(valid_leaders))
        random.shuffle(members)
        teams = {leader: [leader] for leader in valid_leaders}
        for i, member in enumerate(members):
            teams[valid_leaders[i % len(valid_leaders)]].append(member)
        embed = discord.Embed(title="👑 リーダー制チーム分け結果", color=0xF1C40F)
        for l, mem in teams.items():
            embed.add_field(
                name=f"**{l.display_name}**チーム ({len(mem)}人)",
                value="\n".join(f"・{m.display_name}" for m in mem),
                inline=False,
            )
        await interaction.channel.send(
            f"**チーム分け結果** (参加者: {len(participants)}人)", embed=embed
        )

    async def _execute_number_team_split(
        self, interaction: discord.Interaction, num_teams: int
    ):
        participants = await self._close_recruitment(
            interaction, "チーム分けを実行しました！"
        )
        ...
        if len(participants) < num_teams:
            return await interaction.followup.send(
                f"参加者（{len(participants)}人）を{num_teams}チームに分けることはできません。",
                ephemeral=True,
            )
        ...
        for i, mem in enumerate(teams):
            embed.add_field(
                name=f"**チーム{i + 1}** ({len(mem)}人)",
                value="\n".join(f"・{m.display_name}" for m in mem),
                inline=False,
            )
        await interaction.channel.send(
    ...
    async def _execute_start_swiss(self, interaction: discord.Interaction, rounds: int):
        ...
        self.db.execute(
            "INSERT INTO swiss_tournaments (guild_id, is_active, round_num, max_rounds) VALUES (?, ?, ?, ?)",
            (guild_id, True, 1, rounds if rounds > 0 else 0),
        )
        ...
        player_data = [
            (
                ...
                p.id,
                p.display_name,
                isinstance(p.member, DummyPlayer),
                p.score,
                json.dumps(list(p.opponents)),
                p.byes,
                p.wins,
                p.losses,
                p.matches_played,
        ...
        if player_data:
            self.db.execute(
                "INSERT INTO event_players (guild_id, user_id, display_name, is_dummy, score, opponents, byes, wins, losses, matches_played) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                player_data,
            )

        start_msg = (
            f"**スイスドロー大会を開始します！** (参加者: {len(participants)}人)"
        )
        start_msg += (
            f" 全{rounds}ラウンドです。"
            if rounds > 0
            else " 全勝者が1人になるまで続きます。"
        )

        main_ch, match_ch = await self._get_channels(guild_id)
        if not main_ch:
            main_ch = interaction.channel
        if not match_ch:
            match_ch = interaction.channel

        await main_ch.send(start_msg)
        await self.generate_and_send_pairings(interaction.guild, main_ch, match_ch)

    async def generate_and_send_pairings(
        self,
        guild: discord.Guild,
        ...
        if pairings:
            db_pairings = [
                (guild.id, p1.id, p2.id if p2 else None) for p1, p2 in pairings
            ]
            self.db.execute(
                "INSERT INTO swiss_pairings (guild_id, player1_id, player2_id) VALUES (?, ?, ?)",
                db_pairings,
            )
        ...
        for p1, p2 in pairings:
            # 修正: テーブル名 event_players
            self.db.execute(
                "UPDATE event_players SET opponents = ? WHERE guild_id = ? AND user_id = ?",
                (json.dumps(list(p1.opponents)), guild.id, p1.id),
            )
            if p2:
                self.db.execute(
                    "UPDATE event_players SET opponents = ? WHERE guild_id = ? AND user_id = ?",
                    (json.dumps(list(p2.opponents)), guild.id, p2.id),
                )
            else:
                self.db.execute(
                    "UPDATE event_players SET byes = ?, wins = ?, score = ? WHERE guild_id = ? AND user_id = ?",
                    (p1.byes, p1.wins, p1.score, guild.id, p1.id),
                )

        sorted_pairings = sorted(
            pairings,
            key=lambda pair: (
                pair[0].id if pair[1] is None else min(pair[0].id, pair[1].id)
            ),
        )
        desc = "\n".join(
            f"**試合 {i + 1}**: {p1.display_name} ({p1.record}) ⚔️ {p2.display_name} ({p2.record})"
            if p2
            else f"**不戦勝**: {p1.display_name} ({p1.record}) 🏆"
            for i, (p1, p2) in enumerate(sorted_pairings)
        )
        embed = discord.Embed(
            title=f"ラウンド {tournament.round_num} 対戦表",
            description=desc or "対戦カードがありません。",
            color=discord.Color.gold(),
        )
        await main_channel.send(embed=embed)

        if main_channel != match_channel:
            await main_channel.send(
                f"対戦カードを <#{match_channel.id}> に送信しました。"
            )

        for i, (p1, p2) in enumerate(sorted_pairings):
            if p2:
                embed = discord.Embed(
                    title=f"試合 {i + 1}：{p1.display_name} vs {p2.display_name}",
                    description="対戦後、勝者が自身の勝利ボタンを押してください。",
                    color=0x3498DB,
                )
    ...
    async def _handle_swiss_win_logic(
        self,
        interaction: discord.Interaction,
        guild_id: int,
        winner_id: int,
        ...
        if not tournament or not tournament.is_active:
            return await interaction.followup.send(
                "大会は進行中ではありません。", ephemeral=True
            )

        winner = tournament.get_player(winner_id)
        loser = tournament.get_player(loser_id)
        if not winner or not loser:
            return await interaction.followup.send(
                "プレイヤーが見つかりません。", ephemeral=True
            )
        if tournament.is_match_reported(winner.id, loser.id):
            return await interaction.followup.send(
                "この試合は既に結果が報告されています。", ephemeral=True
            )
        ...
        self.db.execute(
            "UPDATE event_players SET score=?, wins=?, matches_played=? WHERE guild_id=? AND user_id=?",
            (winner.score, winner.wins, winner.matches_played, guild_id, winner_id),
        )
        self.db.execute(
            "UPDATE event_players SET losses=?, matches_played=? WHERE guild_id=? AND user_id=?",
            (loser.losses, loser.matches_played, guild_id, loser_id),
        )
        self.db.execute(
            "INSERT INTO swiss_results (guild_id, round_num, winner_id, loser_id) VALUES (?, ?, ?, ?)",
            (guild_id, tournament.round_num, winner_id, loser_id),
        )

        await interaction.followup.send(
            f"✅ {winner.display_name} が {loser.display_name} に勝利しました！",
            ephemeral=True,
        )

        embed = discord.Embed(
            title=f"試合結果: {winner.display_name}の勝利！",
            color=discord.Color.green(),
        )
        embed.set_footer(text="結果報告済み (修正可能)")
        view = SwissResultReportView(guild_id, winner, loser)
        for item in view.children:
            if isinstance(item, discord.ui.Button) and item.custom_id.startswith(
                "swiss_win"
            ):
                item.disabled = True
        ...
        actual_matches = len(
            [p for p, opp in tournament.current_pairings if opp is not None]
        )
        if len(tournament.reported_matches_this_round) >= actual_matches:
            view = NextRoundView()
            await interaction.channel.send(
                "このラウンドの全試合結果が報告されました。\nボタンを押して次のラウンドへ進んでください。",
                view=view,
            )

    async def _handle_swiss_undo_logic(
        self, interaction: discord.Interaction, guild_id: int, p1_id: int, p2_id: int
    ):
        """スイスドローの試合結果報告を取り消す"""
        tournament = self.swiss_tournaments.get(guild_id)
        if not tournament or not tournament.is_active:
        ...
        p1 = tournament.get_player(p1_id)
        p2 = tournament.get_player(p2_id)
        if not p1 or not p2:
            return await interaction.followup.send(
                "プレイヤーが見つかりません。", ephemeral=True
            )
        ...
        report = next(
            (
                r
                for r in tournament.reported_matches_this_round
                if {r[0], r[1]} == {p1.id, p2.id}
            ),
            None,
        )
        if not report:
            return await interaction.followup.send(
                "この試合の結果はまだ報告されていません。", ephemeral=True
            )
        ...
        self.db.execute(
            "DELETE FROM swiss_results WHERE guild_id = ? AND round_num = ? AND winner_id = ? AND loser_id = ?",
            (guild_id, tournament.round_num, winner_id, loser_id),
        )
        if winner:
            self.db.execute(
                "UPDATE event_players SET score=?, wins=?, matches_played=? WHERE guild_id=? AND user_id=?",
                (winner.score, winner.wins, winner.matches_played, guild_id, winner.id),
            )
        if loser:
            self.db.execute(
                "UPDATE event_players SET losses=?, matches_played=? WHERE guild_id=? AND user_id=?",
                (loser.losses, loser.matches_played, guild_id, loser.id),
            )

        await interaction.followup.send(
            f"✅ {p1.display_name}と{p2.display_name}の試合結果を取り消しました。",
            ephemeral=True,
        )

        embed = discord.Embed(
            title=f"試合：{p1.display_name} vs {p2.display_name}",
            description="対戦後、勝者が自身の勝利ボタンを押してください。",
    ...
    async def _execute_next_round(self, interaction: discord.Interaction):
        ...
        if len(tournament.reported_matches_this_round) < actual_matches:
            return await interaction.followup.send(
                "まだ報告されていない試合があります。", ephemeral=True
            )
        ...
        if one_winner or no_winners or final_round:
            ...
            self._archive_tournament(
                guild_id, "スイスドロー大会", "swiss", winner_name, rankings
            )
            # ▲▲▲

            # 修正: テーブル名 swiss_tournaments, event_players, swiss_pairings, swiss_results
            self.db.execute(
                "DELETE FROM swiss_tournaments WHERE guild_id = ?", (guild_id,)
            )
            self.db.execute("DELETE FROM event_players WHERE guild_id = ?", (guild_id,))
            self.db.execute(
                "DELETE FROM swiss_pairings WHERE guild_id = ?", (guild_id,)
            )
        ...
        # 修正: テーブル名 swiss_tournaments
        self.db.execute(
            "UPDATE swiss_tournaments SET round_num = ? WHERE guild_id = ?",
            (tournament.round_num, guild_id),
        )

        await main_ch.send(f"**第 {tournament.round_num} ラウンドを開始します！**")
        await self.generate_and_send_pairings(interaction.guild, main_ch, match_ch)

    async def _display_standings(
        self,
        interaction: discord.Interaction,
        final: bool = False,
        ...
        ranked_players = tournament.get_ranked_players()
        embed = discord.Embed(
            title=f"📊 スイスドロー大会 {'最終' if final else '現在'}順位",
            color=0xDAA520,
        )
        standings = [
            f"{i + 1}. **{p.display_name}** ({p.record}) - OMW%: {p.omw:.2f}"
            for i, p in enumerate(ranked_players)
        ]
    ...
    async def _execute_bracket(self, interaction: discord.Interaction):
        ...
        async with self.locks[guild_id]:
            participants_set = await self._close_recruitment(
                interaction, "トーナメントを開始しました！"
            )
            ...
            self.db.execute(
                "DELETE FROM se_tournaments WHERE guild_id = ?", (guild_id,)
            )
            self.db.execute("DELETE FROM event_players WHERE guild_id = ?", (guild_id,))
            self.db.execute(
                "INSERT INTO se_tournaments (guild_id, is_active, num_players, num_rounds, bracket_message_id) VALUES (?, ?, ?, ?, ?)",
                (guild_id, True, num_players, num_rounds, None),
            )

            player_db_data = [
                (guild_id, p.id, p.display_name, isinstance(p, DummyPlayer))
                for p in participants_set
            ]
            if player_db_data:
                self.db.execute(
                    "INSERT INTO event_players (guild_id, user_id, display_name, is_dummy) VALUES (?, ?, ?, ?)",
                    player_db_data,
                )

            player_list_for_seeding = [
                {"id": p.id, "name": p.display_name, "member": p}
                for p in participants_set
            ]
            byes = [
                {"id": None, "name": "(不戦勝)", "is_bye": True}
            ...
            while i < tourney_size:
                if i + 1 < len(seeding_list):
                    ...
                    if p1.get("is_bye") and p2.get("is_bye"):
                        swap_target_idx = next(
                            (
                                j
                                for j in range(i + 2, tourney_size)
                                if not seeding_list[j].get("is_bye")
                            ),
                            -1,
                        )
                        if swap_target_idx != -1:
                            seeding_list[i + 1], seeding_list[swap_target_idx] = (
                                seeding_list[swap_target_idx],
                                seeding_list[i + 1],
                            )
                        else:
                            log.warning(
                                f"Guild {guild_id}: 不戦勝ペアを解消できず再シャッフルします。"
                            )
                            random.shuffle(seeding_list)
                            i = 0
                            continue
            ...
            matches_to_insert = []
            next_round_source_matches = []
            for i in range(0, tourney_size, 2):
                p1_data, p2_data = seeding_list[i], seeding_list[i + 1]
                match_id = f"R1M{i // 2}"
                is_bye_match = p1_data.get("is_bye", False) or p2_data.get(
                    "is_bye", False
                )
                winner_id = (
                    p2_data["id"]
                    if p1_data.get("is_bye")
                    else (p1_data["id"] if p2_data.get("is_bye") else None)
                )
            ...
            for r_num in range(2, num_rounds + 1):
                sources = list(next_round_source_matches)
                next_round_source_matches = []
                for i in range(0, len(sources), 2):
                    p1_source, p2_source = sources[i], sources[i + 1]
                    match_id = f"R{r_num}M{i // 2}"
                    matches_to_insert.append(
                        (
                            guild_id,
                            match_id,
            ...
            if matches_to_insert:
                self.db.execute(
                    "INSERT INTO se_matches VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    matches_to_insert,
                )

            for bye_match in self.db.fetchall(
                "SELECT match_id, winner_id FROM se_matches WHERE guild_id = ? AND round_num = 1 AND is_bye = 1",
                (guild_id,),
            ):
                next_match = self.db.fetchone(
                    "SELECT match_id, player1_source_match_id FROM se_matches WHERE guild_id = ? AND (player1_source_match_id = ? OR player2_source_match_id = ?)",
                    (guild_id, bye_match["match_id"], bye_match["match_id"]),
                )
                if next_match:
                    column_to_update = (
                        "player1_id"
                        if next_match["player1_source_match_id"]
                        == bye_match["match_id"]
                        else "player2_id"
                    )
                    self.db.execute(
                        f"UPDATE se_matches SET {column_to_update} = ? WHERE guild_id = ? AND match_id = ?",
                        (bye_match["winner_id"], guild_id, next_match["match_id"]),
                    )

            await interaction.followup.send("トーナメント表を作成中...", ephemeral=True)
            image_file = await self.bot.loop.run_in_executor(
                None, create_bracket_image_from_db, guild_id, self.db
            )
            ...
            if image_file:
                bracket_msg = await main_ch.send(
                    f"**トーナメント開始！** (参加者: {num_players}人)", file=image_file
                )
                self.db.execute(
                    "UPDATE se_tournaments SET bracket_message_id = ? WHERE guild_id = ?",
                    (bracket_msg.id, guild_id),
                )
            else:
                await main_ch.send("トーナメント表の生成に失敗しました。")
    ...
    async def _handle_se_win_logic(
        self,
        interaction: discord.Interaction,
        guild_id: int,
        match_id: str,
        ...
        async with self.locks[guild_id]:
            try:
                if not self.db.fetchone(
                    ...
                    return await interaction.followup.send(
                        "トーナメントは進行中ではありません。", ephemeral=True
                    )
                current_match = self.db.fetchone(
                    "SELECT winner_id FROM se_matches WHERE guild_id = ? AND match_id = ?",
                    (guild_id, match_id),
                )
                if current_match and current_match["winner_id"] is not None:
                    return await interaction.followup.send(
                        "この試合は既に結果が報告されています。", ephemeral=True
                    )

                self.db.execute(
                    "UPDATE se_matches SET winner_id = ? WHERE guild_id = ? AND match_id = ?",
                    (winner_id, guild_id, match_id),
                )
                await interaction.followup.send(
                    "勝利報告を受け付けました！", ephemeral=True
                )
                # 修正: テーブル名 event_players
                p_map = {
                    p["user_id"]: p["display_name"]
                    for p in self.db.fetchall(
                        "SELECT user_id, display_name FROM event_players WHERE guild_id = ?",
                        (guild_id,),
                    )
                }
                embed = discord.Embed(
                    title=f"結果: {p_map.get(winner_id, '不明')} の勝利！",
                    color=discord.Color.green(),
                )
                await interaction.message.edit(embed=embed, view=None)

                next_match = self.db.fetchone(
                    "SELECT * FROM se_matches WHERE guild_id = ? AND (player1_source_match_id = ? OR player2_source_match_id = ?)",
                    (guild_id, match_id, match_id),
                )
                main_ch, match_ch = await self._get_channels(guild_id)
                if not main_ch:
                    main_ch = interaction.channel
                ...
                if next_match:
                    ...
                    column = (
                        "player1_id"
                        if next_match["player1_source_match_id"] == match_id
                        else "player2_id"
                    )
                    self.db.execute(
                        f"UPDATE se_matches SET {column} = ? WHERE guild_id = ? AND match_id = ?",
                        (winner_id, guild_id, next_match_id),
                    )
                    updated_next_match = self.db.fetchone(
                        "SELECT * FROM se_matches WHERE guild_id = ? AND match_id = ?",
                        (guild_id, next_match_id),
                    )
                    if (
                        updated_next_match["player1_id"]
                        and updated_next_match["player2_id"]
                    ):
                        p1 = await self._resolve_se_player(
                            guild_id, updated_next_match["player1_id"]
                        )
                        p2 = await self._resolve_se_player(
                            guild_id, updated_next_match["player2_id"]
                        )
                        if not p1 or not p2:
                            raise ValueError(
                                f"次ラウンドの参加者解決に失敗しました: {next_match_id}"
                            )
                        embed = discord.Embed(
                            title=f"【第{updated_next_match['round_num']}回戦】 {p1.display_name} vs {p2.display_name}",
                            description="対戦の準備ができました。",
                        )
                        view = ResultReportView(
                            guild_id, next_match_id, Player(p1), Player(p2)
                        )
                        await match_ch.send(embed=embed, view=view)
                        if main_ch.id != match_ch.id:
                            await main_ch.send(
                                f"新しい対戦カードが <#{match_ch.id}> に作成されました。"
                            )
                ...
                if image_file:
                    tourney_info = self.db.fetchone(
                        "SELECT bracket_message_id FROM se_tournaments WHERE guild_id = ?",
                        (guild_id,),
                    )
                    bracket_message_id = (
                        tourney_info["bracket_message_id"] if tourney_info else None
                    )
                    try:
                        if bracket_message_id is None:
                            raise ValueError("bracket_message_id is None")
                        msg_to_edit = await main_ch.fetch_message(
                            int(bracket_message_id)
                        )
                        await msg_to_edit.edit(
                            content="**トーナメント表更新**", attachments=[image_file]
                        )
                    except (
                        ValueError,
                        TypeError,
                        ...
                        new_msg = await main_ch.send(
                            "**トーナメント表更新**", file=image_file
                        )
                        self.db.execute(
                            "UPDATE se_tournaments SET bracket_message_id = ? WHERE guild_id = ?",
                            (new_msg.id, guild_id),
                ...
                if not next_match:
                    ...
                    self._archive_tournament(
                        guild_id, "トーナメント", "se", winner_name, rankings
                    )
                    # ▲▲▲
                    ...
                    self.db.execute(
                        "DELETE FROM se_matches WHERE guild_id = ?", (guild_id,)
                    )
                    self.db.execute(
                        "DELETE FROM event_players WHERE guild_id = ?", (guild_id,)
                    )

            except Exception as e:
                log.error(
                    f"勝利処理中にエラーが発生(Guild: {guild_id}, Match: {match_id}): {e}",
                    exc_info=True,
                )
                await interaction.followup.send(
                    f"エラーが発生しました。Botの管理者にご連絡ください。",
                    ephemeral=True,
                )

    @commands.command(
        name="中止", aliases=["sd中止"], help="進行中の募集や大会を強制的に中止します。"
    )
    @commands.has_permissions(manage_guild=True)
    async def cancel_command(self, ctx: commands.Context):
        view = ConfirmCancelView(self)
        msg = await ctx.send(
            "本当に現在の募集または大会を中止しますか？この操作は取り消せません。",
            view=view,
        )
        view.message = msg
    ...
    async def _execute_cancel(self, interaction: discord.Interaction):
        ...
        await interaction.message.edit(
            content="🚨 募集または大会を中止し、データをリセットしました。", view=None
        )

    @commands.command(
        name="ヘルプ3",
        aliases=["sdヘルプ"],
        help="大会・チーム分け機能の詳細なヘルプを表示します。",
    )
    async def tournament_help(self, ctx: commands.Context):
        embed = discord.Embed(
            title="⚔️ 大会・チーム分け機能 詳細ヘルプ",
            description="イベントの開催から終了までの流れと、各機能の使い方を説明します。",
            color=0x4CAF50,
        )
        embed.add_field(
            name="【ステップ1】参加者を募集する",
            value="**1. `!募集` コマンドを実行します。**\n   - Botが参加者募集用のメッセージと操作パネルを投稿します。\n**2. 参加者は👍リアクションを押します。**\n   - 参加をキャンセルする場合はリアクションを外します。",
            inline=False,
        )
        embed.add_field(
            name="【ステップ2】イベントを開始する",
            value="操作パネルのボタンを押して、希望のイベントを開始します。\n\n**< トーナメント / スイスドロー >**\n- ボタンを押すと大会が開始され、対戦カードが投稿されます。\n- 試合が終わったら、勝者が自分の勝利ボタンを押します。\n- トーナメントは勝者が次の試合へ、スイスドローは全員が次のラウンドへ進みます。",
            inline=False,
        )
        embed.add_field(
            name="【その他コマンド】(要管理者権限)",
            value="**`!追加 [人数]`**: 募集中にダミー参加者を追加します。\n**`!削除 [人数]`**: 募集中にダミー参加者を削除します。\n**`!中止`**: 進行中の募集や大会を強制的に中止します。\n**`!sd設定メイン`**: 大会アナウンス用チャンネルを設定します。\n**`!sd設定対戦`**: **トーナメントとスイスドロー**の対戦カードを投稿するチャンネルを設定します。",
            inline=False,
        )
